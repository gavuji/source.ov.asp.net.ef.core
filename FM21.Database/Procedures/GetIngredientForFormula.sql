CREATE PROCEDURE GetIngredientForFormula
( 
...
BEGIN
	...
	SET @selectStatement = ', IM.IngredientID';
	SET @fromStatement = 'FROM IngredientMaster IM
			LEFT JOIN NutrientMaster NM ON NM.[Name] = IM.ActiveNutrient
			LEFT JOIN RegulatoryMaster RM ON RM.NutrientId = NM.NutrientID';

	IF(ISNULL(@IngredientIDs, '') = '')
	BEGIN
		SET @whereStatement = 'WHERE IM.IngredientID = 0';
	END
	ELSE
	BEGIN
		SET @whereStatement = 'WHERE IM.IngredientID IN (' + @IngredientIDs + ')';
	END
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SET @selectStatement += ', ' + (SELECT [dbo].GetColumnByParams(@col,'IngredientMaster', 0)) 
	...
	SET @selectStatement += ', RM.NewUsRdi AS DvOfActiveNutrient, CASE WHEN RM.RegulatoryID IS NOT NULL THEN 1 ELSE 0 END AS IsDVValue';
